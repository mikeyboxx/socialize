...
const {apiDaemon, apiCleanupDaemon} = require('./utils/apiDaemon'); // bots to create/delete rows on the DB
const {formatDate, isEqual} = require('./utils/helpers');  // handlebars helper functions
const routes = require('./controllers');  // business logic for every ajax request
...
sequelize.sync({ force: false }).then(() => {
  app.listen(PORT, ()=>{
    // generate random api content which is saved on the Post table
    apiDaemon(); 

    // delete api generated posts from the Post table, that are x days/hours/mins old
    apiCleanupDaemon();
